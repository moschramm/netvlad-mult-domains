...
load_size = 286
root_dir = '/home/moritz/BA/pytorch-NetVlad/Pittsburgh/'
...
def input_transform(arch='todaygan'):
    if arch == 'todaygan':
...
def get_training_query_set(arch, margin=0.1):
    ...
    return QueryDatasetFromStruct(structFile,
                             input_transform=input_transform(arch), margin=margin)
...
def get_250k_val_query_set(arch):
    structFile = join(struct_dir, 'pitts250k_val.mat')
    return QueryDatasetFromStruct(structFile,
                             input_transform=input_transform(arch))
...
class QueryDatasetFromStruct(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        query = Image.open(join(queries_dir, self.dbStruct.qImage[index])).convert('RGB')
