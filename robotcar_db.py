"""
A module used for building a dataset database images only without using any
positional ground truth.
...
def get_whole_val_set(arch):
    ...
    return TestDatasetFromStruct(structFile,
                             input_transform=input_transform(arch))
...
class WholeDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, input_transform=None, onlyDB=False):
        ...
        if not onlyDB:
            self.images += \
                [join(queries_dir, qIm[:19] + '_GAN' \
                + qIm[19:]) for qIm in self.dbStruct.dbImage]
    ...
    def getPositives(self):
        ...
        if self.positives is None:
            self.positives = np.arange(self.dbStruct.numDb)

        return self.positives

class TestDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, input_transform=None, onlyDB=False):
        super().__init__()
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(self, structFile, nNegSample=1000, nNeg=10, margin=0.1,
        ...
        # positives are the same images, but before translation
        # they therefore have the same index
        self.positives = np.arange(self.dbStruct.numDb)

        self.queries = np.arange(self.dbStruct.numDb)

        # filepath of HDF5 containing feature vectors for images
        self.cache = None

        self.negCache = [np.empty((0,)) for _ in range(self.dbStruct.numDb)]
    ...
    def __getitem__(self, index):
        with h5py.File(self.cache, mode='r') as h5:
            ...
            # index of positive in db is the same as index of query in queries
            posIndex = index
            posFeat = h5feat[posIndex]
            dPos = np.linalg.norm(posFeat - qFeat.reshape(1,-1))

            # negatives are everything wich is not a positive
            negSample = np.random.choice(
                np.delete(self.positives, index), self.nNegSample)
            negSample = np.unique(
        ...
        query = Image.open(join(
            queries_dir,
            self.dbStruct.dbImage[index][:19] + '_GAN' \
            + self.dbStruct.dbImage[index][19:])).convert('RGB')
        positive = Image.open(
