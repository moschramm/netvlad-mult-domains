"""
A module used for building a dataset of database images only using positional
ground truth.
...
def get_training_query_set(arch, margin=0.1):
    structFile = [join(
        ...
        'robotcar_train_db_' + str(set_id) \
        + '.mat') for set_id in range(nDatasetSteps)]
    return [QueryDatasetFromStruct(file,input_transform=input_transform(arch),
